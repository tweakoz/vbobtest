...
const char* Usage = "\n"
              ...
              "\tvalue            control value: run mlquery -d device -j jack -v "
              "control\n"
...
int main(int argc, char** argv)
{
    ...
    } else if (mluFindFirstInputJack(devId, &jackId) && mluFindFirstOutputJack(devId, &jackId)) {
    ...
    if (mluFindPathFromJack(jackId, &pathId, NULL) && mluFindPathToJack(jackId, &pathId, NULL)) {
...
int print_controls(MLint64 openPath, FILE* f)
{
    ...
    for (i = 0; i < paramIds->length; i++) {
        MLint32 clazz = ML_PARAM_GET_CLASS(paramIds->value.pInt64[i]);
        if (clazz == ML_CLASS_VIDEO || clazz == ML_CLASS_IMAGE || clazz == ML_CLASS_AUDIO) {
            int vs, ps;
            MLpv* paramCap;
            MLpv controls[2];
            MLstatus stat;

            if (paramIds->value.pInt64[i] == ML_VIDEO_UST_INT64 || paramIds->value.pInt64[i] == ML_VIDEO_MSC_INT64 || paramIds->value.pInt64[i] == ML_VIDEO_ASC_INT64 || paramIds->value.pInt64[i] == ML_AUDIO_UST_INT64 || paramIds->value.pInt64[i] == ML_AUDIO_MSC_INT64 || paramIds->value.pInt64[i] == ML_AUDIO_ASC_INT64) {
                continue;
            }
            ...
            if (stat = mlGetControls(openPath, controls)) {
                if (debug) {
                    ...
                    fprintf(stderr, "Unable to get param %s: %s", pname,
                        mlStatusName(stat));
                    continue;
                }
            }
            ...
            if (mlPvValueToString(openPath, controls, valueBuffer, &vs)) {
                fprintf(stderr, "Unable to convert %s value %d (0x%x) to string",
                    paramBuffer, controls[0].value.int32, controls[0].value.int32);
                continue;
            }
            if (ps + vs < 72) {
