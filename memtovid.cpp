...
int main(int argc, char **argv)
{
	...
	case 's':
	  timing_option = optarg;
	     ...
	     !strcmp(optarg, "480i") || !strcmp(optarg, "486i")) {
    ...
    */
    printf( "Prerolling %d buffers: ", maxBuffers ); fflush( stdout );
		     ...
		     MLint32 maxBuffers)
{
  ...
  if( fileName ) printf( "from %s: ", fileName );
  fflush(stdout);

  if( !fileName ) for (i = 0; i < maxBuffers; i++) 
    {
      MLbyte* p;
      int x,y;
	...
	int rc;
	if( imagefiles ) {
	    char fn[100]; sprintf( fn, fileName, i );
	    if( ppmfile ) {
		fp = fopen( fn, "rb" );
		if( fp < 0 ) {perror( fn ); return -1; }
	    }
	    else {
	...
	if( ppmfile ) {
	    ...
	    if( rc != imageSize ) { perror( "read" ); return -1; }
	}
